layui.extend({}).define(['md5', 'febs', 'layer', 'upload'], function(exports) {
    var $ = layui.$,
        febs = layui.febs,
        md5 = layui.md5,
        layer = layui.layer,
    ...
    Class.prototype.uploadFile = function(filess, filekey, fileCount, data) {
        ...
        var dir = this.options.prefixPath + new Date().getTime() + '/' + (Math.random() + "").substring(2, 7) + '/';

        var name = data.file.name; 
        var ext = name.substr(name.lastIndexOf("."));
        var time = new Date().getTime();
        name = md5(name + time + Math.ceil(Math.random() * 10000)) + ext;

        multipleFileKeyArray.push(dir + name);
        ...
        $.ajax({
            ...
            success: function() {
                var result = {
                    ...
                    ossUrl: httpStr+'://'+bucket + '.' + region + '/' + multipleFileKeyArray[successCount]
                };
                //成功无返回
                if (multipleState) {
