...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var post = function(options, callback, tips){
    ...
    return $.ajax({
      ...
      ,success: function(res){
        ...
        : layer.msg(res.message || ((tips||'Error') + ': LAYIM_NOT_GET_DATA'), {
          time: 5000
        });
      },error: function(err, message){
        window.console && console.log && console.error('LAYIM_DATE_ERROR：' + message);
      }
    });
  };
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      // ,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title||'<i class="layui-icon">&#xe943;</i>') +'</span>'
      ,content: '<span>'+ (content||cache.base.title||'<i style="color: #1890ff!important; font-size: 28px;" class="layui-icon">&#xe943;</i>') +'</span>'
  ...
  var anim = 'layui-anim-up', events = {
    ...
    //关于
    ,about: function(){
      layer.alert('版本： '+ v + '<br>版权所有：layim.layui.com', {
        title: '关于 LayIM'
        ,shade: false
      });
    }
