...
class RSA
{
    ...
    {
        $this->privateKey = openssl_pkey_get_private(file_get_contents(ROOT . '/pem/rsa_private.pem'));//私钥，用于加密
        $this->publicKey = openssl_pkey_get_public(file_get_contents(ROOT . '/pem/rsa_public.pem'));//公钥，用于解密
    }
    ...
    public function encryptByPrivateKey($data) 
    {
        ...
        foreach (str_split($data, 117) as $chunk) {
            openssl_private_encrypt($chunk, $encrypted, $this->privateKey, $this->padding);//私钥加密
            $crypto .= $encrypted;
        }
     ...
     */
    public function decryptByPrivateKey($data, $js = false)
    {
        if ($js) {
        ...
        foreach (str_split($this->urlsafe_b64decode($data), 128) as $chunk) {
            openssl_private_decrypt($chunk, $encrypted, $this->privateKey, $this->padding);//私钥解密

            if ($js) {
                $encrypted = rtrim(strrev($encrypted), "\0");
            }
    ...
    public function encryptByPublicKey($data) 
    {
        $crypto = '';
        foreach (str_split($data, 117) as $chunk) {
            openssl_public_encrypt($chunk, $decrypted, $this->publicKey, $this->padding);//公钥加密
            $crypto .= $decrypted;
        }
    ...
    public function decryptByPublicKey($data) 
    {
        $crypto = '';
        foreach (str_split($this->urlsafe_b64decode($data), 128) as $chunk) {
            openssl_public_decrypt($chunk, $decrypted, $this->publicKey, $this->padding);//公钥解密
            $crypto .= $decrypted;
        }
